...
I_ant0 = np.zeros(len(partition))
I_ant1 = np.zeros(len(partition))
I_ant2 = np.zeros(len(partition))
I_ant3 = np.zeros(len(partition))
for i in range(len(partition)):
    nss = ns_master_curr[i]
    I_ant0[i], _ = rate0.find_desired_Idc(nss, I_guess, option = 'nanoLED')
    I_ant1[i], _ = rate1.find_desired_Idc(nss, I_guess, option = 'nanoLED')
    I_ant2[i], _ = rate2.find_desired_Idc(nss, I_guess, option = 'nanoLED')
    I_ant3[i], _ = rate3.find_desired_Idc(nss, I_guess, option = 'nanoLED')
...
if RANK == 0:
    ...
    outfile = 'data_WDM_transfer8_fracs'
